...
let str = 'Температура воздуха составляет 100.2 градуса по цельсию';
...
let newStr = str9.replace(re4, function(match){ 
    //console.log({match}); 
...
let result = str10.replace(/(\w+) (\w+)/,  function(match, name1, name2){
  return `${name2}, ${name1}`
});
