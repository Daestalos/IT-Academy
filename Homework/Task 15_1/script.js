...
// Task 6. При клике на кнопку "Посчитать сумму всех товаров" умножьте количество выбранных товаров на их сумму. Количество выбранного товара можно изменять при клике на кнопки "+1" и "-1".(По желанию, добавьте проверку, чтобы нельзя было выбрать меньше 0 товаров.)

let currentCartItems = document.querySelector('#current-cart-items'),
    totalBtn = document.querySelectorAll('#current-cart-items .btn')[8],
    totalSpan = document.querySelector('#current-total-items');

currentCartItems.addEventListener('click', (e) => {
    let currentBtn = e.target.dataset,
    ...
    if (currentBtn.btn === 'btn-pos' ) {
        Number(++previousInputForPlus.value)
    }
})
...
let inputValue = document.querySelector('input[name="input-info"]'),
    inputDiv = inputValue.closest('div');

document.addEventListener('keydown', (e) => {
    if (e.key === 'Enter'){
        inputDiv.insertAdjacentHTML('beforeend', `<p>${inputValue.value}</p>`)
        inputValue.value = ''
    }
})
