...
// N.20 Домашнее задание DRINKS_HASH_CLASS
// Создать проект DRINKS_HASH_CLASS по аналогии с проектом DRINKS_HASH_FUNC, только:
// ●	класс назвать HashStorageClass;
// ●	класс должен быть описан в ES6-стиле (ключевым словом class);
// ●	пары ключ-значение должны храниться в публично доступном хэше.
...
class HashStorageClass{
    ...
    getValue(key){
        return drinkStorage[key] ? `Напиток "${key}" ${drinkStorage[key]}` : 'Информация о напитке отсутствует' 
    }
    deleteValue(key){
        if (drinkStorage[key]){
